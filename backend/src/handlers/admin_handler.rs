...
use crate::application::usecases::admin_usecase::{CreateCategoryCmd, UpdateCategoryCmd};
...
pub async fn update_category_handler(
    ...
    let view_policy = body
        ...
        .as_deref()
        .map(|s| {
            parse_view_policy(s).ok_or_else(|| AppError::unprocessable("Invalid view_policy"))
        })
        .transpose()?;
    let post_policy = body
        ...
        .as_deref()
        .map(|s| {
            parse_post_policy(s).ok_or_else(|| AppError::unprocessable("Invalid post_policy"))
        })
        .transpose()?;
...
pub async fn list_category_moderators_handler(
    ...
    let user = auth_user.as_ref().ok_or(AppError::Unauthorized)?;
    let pairs = state.admin.list_category_moderators_with_users(user, category_id).await?;
...
pub async fn assign_moderator_handler(
    ...
    let mod_user = state.admin.users.find_by_id(assignment.user_id).await?.ok_or(AppError::NotFound)?;
    Ok((
        StatusCode::CREATED,
