...
use crate::constants::{MAX_FAVICON_BYTES, MAX_LOGO_BYTES};
use crate::infrastructure::storage::cas::{cas_key, validate_favicon_content_type, validate_image_content_type};
...
pub async fn get_public_config_handler(
    ...
    let public: HashMap<String, String> = all
        .into_iter()
        .filter(|(k, _)| matches!(k.as_str(), "site_name" | "site_tagline" | "logo_url" | "favicon_url" | "primary_color"))
        .collect();
...
pub async fn update_site_config_handler(
    ...
    let allowed_keys = [
        ...
        "primary_color", "registration_open", "keyword_blacklist",
    ];
...
pub async fn upload_favicon_handler(
        ...
        .map_err(|e| AppError::UnprocessableEntity(e.to_string()))?
    {
        if field.name() == Some("file") {
            ...
            if !validate_favicon_content_type(&ct) {
                return Err(AppError::unprocessable("favicon must be ico, svg, png, gif, or jpeg").into());
            }
    ...
    if state.stored_files.exists(&key).await? {
        ...
        state.stored_files.upsert(&key, &content_type, &data, data.len() as i64, Some(actor.id)).await?;
    }

    // Swap out old favicon and release its ref
    let old_key = state.site_config.get("favicon_url").await?
    ...
    Ok(Json(serde_json::json!({ "data": { "favicon_url": favicon_url } })))
}
...
pub async fn upload_logo_handler(
    ...
    let old_key = state.site_config.get("logo_url").await?
        .and_then(|url| url.strip_prefix("/files/").map(str::to_string));
    ...
    Ok(Json(serde_json::json!({ "data": { "logo_url": logo_url } })))
}
...
pub async fn delete_logo_handler(
    ...
    if let Some(key) = state.site_config.get("logo_url").await?
...
pub async fn delete_favicon_handler(
    ...
    if let Some(key) = state.site_config.get("favicon_url").await?
        .filter(|url| !url.is_empty())
        .and_then(|url| url.strip_prefix("/files/").map(str::to_string))
    {
        let remaining = state.stored_files.decrement_ref(&key).await?;
