...
pub async fn create_webhook_handler(
    ...
    let actor = auth_user.as_ref().ok_or(AppError::Unauthorized)?;
    let webhook = state.webhook.create(actor, body.url, body.events, body.secret).await?;
    Ok((StatusCode::CREATED, Json(DataResponse::new(WebhookResponse::from(webhook)))))
}
...
pub async fn update_webhook_handler(
    ...
    let actor = auth_user.as_ref().ok_or(AppError::Unauthorized)?;
    let webhook = state.webhook.update(actor, id, body.url, body.events, body.secret, body.is_active).await?;
