...
use crate::application::usecases::moderation_usecase::CreateReportCmd;
...
pub async fn create_report_handler(
    ...
    let report = state
        ...
        .await?;
    Ok((StatusCode::CREATED, Json(DataResponse::new(ReportResponse::from(report)))))
}
...
pub async fn list_mod_reports_handler(
    ...
    let (reports, total) = state.moderation.list_pending_reports(actor, target_type, page, per_page).await?;
...
pub async fn resolve_report_handler(
    ...
    let status = match body.status.as_str() {
        ...
        _ => return Err(AppError::unprocessable("status must be 'resolved' or 'dismissed'").into()),
    };
    state.moderation.resolve_report(actor, id, status, body.moderator_notes).await?;
...
pub async fn list_audit_log_handler(
    ...
    let actor = auth_user.as_ref().ok_or(AppError::Unauthorized)?;
    // Verify mod or admin
    let assigned = state.cat_mod_repo.list_category_ids_for_user(actor.id).await?;
    ...
    Ok(Json(crate::view_models::PagedResponse::new(
        logs.into_iter().map(|l| AuditLogResponse {
            id: l.id,
            actor_id: l.actor_id,
            action: l.action,
