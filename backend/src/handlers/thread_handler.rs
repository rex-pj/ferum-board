...
use crate::application::usecases::thread_usecase::CreateThreadCmd;
...
fn validate_image_field(content_type: &str, data: &bytes::Bytes) -> Result<(), AppError> {
    if !matches!(content_type, "image/jpeg" | "image/png" | "image/webp" | "image/gif") {
    ...
    if data.len() > MAX_SIZE {
        return Err(AppError::UnprocessableEntity("Thumbnail exceeds 2 MB limit".to_string()));
    }
...
pub async fn create_thread_handler(
        ...
        .map_err(|e| AppError::UnprocessableEntity(e.to_string()))?
    {
        match field.name() {
            Some("category_id") => {
                let s = field.text().await.map_err(|e| AppError::UnprocessableEntity(e.to_string()))?;
                category_id = Some(
                    s.parse::<Uuid>()
                        .map_err(|_| AppError::UnprocessableEntity("Invalid category_id".to_string()))?,
                );
            }
            Some("title") => {
                title = Some(field.text().await.map_err(|e| AppError::UnprocessableEntity(e.to_string()))?);
            }
            Some("content_md") => {
                content_md = Some(field.text().await.map_err(|e| AppError::UnprocessableEntity(e.to_string()))?);
            }
            Some("thumbnail") => {
    ...
    let title = title
        .ok_or_else(|| AppError::UnprocessableEntity("title is required".to_string()))?;
    ...
    if title.len() < 5 || title.len() > 255 {
        return Err(AppError::UnprocessableEntity("Title must be 5–255 characters".to_string()).into());
    }
    ...
    let post = state
        ...
        .create(
            actor,
            crate::application::usecases::post_usecase::CreatePostCmd {
                thread_id: thread.id,
                parent_id: None,
                content_md,
            },
    ...
    if let Some((data, content_type)) = thumbnail {
        let url = state.thread.set_thumbnail(actor, thread.id, data, content_type).await?;
...
pub async fn update_thread_handler(
    ...
    if let Some((data, content_type)) = thumbnail {
        let url = state.thread.set_thumbnail(actor, id, data, content_type).await?;
        thread.thumbnail_url = Some(url);
    }
...
pub async fn move_thread_handler(
    ...
    let actor = auth_user.as_ref().ok_or(AppError::Unauthorized)?;
    let assigned = get_assigned(&state, actor.id).await?;
    let thread = state.thread.move_to(actor, id, &assigned, body.category_id).await?;
    Ok(Json(DataResponse::new(ThreadResponse::from(thread))))
}
...
pub async fn solve_thread_handler(
    ...
    let actor = auth_user.as_ref().ok_or(AppError::Unauthorized)?;
    let assigned = get_assigned(&state, actor.id).await?;
    let thread = state.thread.mark_solved(actor, id, body.best_answer_id, &assigned).await?;
    Ok(Json(DataResponse::new(ThreadResponse::from(thread))))
}
...
pub async fn upload_thumbnail_handler(
    ...
    let url = state.thread.set_thumbnail(actor, id, data, content_type).await?;
    Ok(Json(serde_json::json!({ "data": { "thumbnail_url": url } })))
}
