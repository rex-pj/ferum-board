...
use crate::application::usecases::auth_usecase::{LoginCmd, RefreshResult, RegisterCmd, ResetPasswordCmd};
use crate::middleware::AuthUser;
use crate::view_models::auth::{
    ForgotPasswordRequest, LoginRequest, LoginResponse, RefreshResponse, RegisterRequest,
...
pub async fn register_handler(
    ...
    Ok((StatusCode::CREATED, Json(DataResponse::new(UserResponse::from(user)))))
}
...
pub async fn login_handler(
    ...
    let result = state
        .auth
        .login(LoginCmd { email: body.email, password: body.password })
        .await?;
...
pub async fn logout_handler(
    ...
    resp_headers.insert(
        header::SET_COOKIE,
        format!("token=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0{}", secure).parse().unwrap(),
    );
    resp_headers.append(
        header::SET_COOKIE,
...
pub async fn reset_password_handler(
    ...
    state
        .auth
        .reset_password(ResetPasswordCmd { token, new_password: body.new_password })
        .await?;
    ...
    headers
        ...
        .and_then(|cookies| {
            cookies.split(';').find_map(|part| {
                part.trim().strip_prefix("refresh_token=").map(|t| t.to_string())
            })
        })
}
