...
use crate::application::usecases::admin_usecase::AdminUpdateUserCmd;
...
pub async fn admin_update_user_handler(
    ...
    let role = body.role.as_deref().map(|r| match r {
        ...
        _ => Err(AppError::unprocessable("Invalid role. Use: member, moderator, admin")),
    }).transpose()?;

    let user = state.admin.update_user_role(actor, id, AdminUpdateUserCmd {
        role,
