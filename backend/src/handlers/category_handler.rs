...
use crate::application::usecases::category_usecase::{ForumIndexItem, SubcategoryCount};
use crate::middleware::AuthUser;
use crate::view_models::category::{CategoryResponse, ForumIndexGroupResponse, SubcategoryIndexResponse};
...
pub async fn get_category_handler(
    ...
    let category = state.category.get_by_slug(auth_user.as_ref(), &slug).await?;
...
pub async fn get_forum_index_handler(
    ...
    let data: Vec<ForumIndexGroupResponse> = items.into_iter().map(forum_index_item_to_response).collect();
    Ok(Json(DataResponse::new(data)))
}
...
fn forum_index_item_to_response(item: ForumIndexItem) -> ForumIndexGroupResponse {
    ForumIndexGroupResponse {
        ...
        subcategories: item.subcategories.into_iter().map(sub_to_response).collect(),
        recent_threads: item.recent_threads.into_iter().map(ThreadResponse::from).collect(),
    }
}
