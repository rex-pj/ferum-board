...
pub fn build_router(state: AppState, cors_origins: &str) -> Router {
    ...
    let admin_category_routes = Router::new()
        ...
        .route("/{id}", patch(update_category_handler).delete(delete_category_handler))
        .route("/{id}/moderators", get(list_category_moderators_handler).post(assign_moderator_handler))
        .route("/{id}/moderators/{user_id}", delete(revoke_moderator_handler));

    // Extended admin routes
    let admin_user_routes = Router::new()
        .route("/", get(admin_list_users_handler))
        .route("/{id}", get(admin_get_user_handler).patch(admin_update_user_handler))
        .route("/{id}/ban", post(admin_ban_user_handler).delete(admin_unban_user_handler));
    ...
    let admin_routes = Router::new()
        ...
        .route("/config", get(get_site_config_handler).put(update_site_config_handler))
        .route("/config/logo", post(upload_logo_handler).delete(delete_logo_handler))
        .route("/config/favicon", post(upload_favicon_handler).delete(delete_favicon_handler))
        .route("/webhooks", get(list_webhooks_handler).post(create_webhook_handler))
        .route("/webhooks/{id}", patch(update_webhook_handler).delete(delete_webhook_handler))
    ...
    let thread_routes = Router::new()
        ...
        .route("/{slug}", get(get_thread_handler).patch(update_thread_handler).delete(delete_thread_handler))
        ...
        .route("/{slug}/posts", get(list_posts_handler).post(create_post_handler))
        .route("/{slug}/bookmarks", get(get_bookmark_status_handler).post(add_bookmark_handler).delete(remove_bookmark_handler))
        .route("/{id}/thumbnail", post(upload_thumbnail_handler).delete(delete_thumbnail_handler));

    // Post routes (edit, delete, reactions)
    let post_routes = Router::new()
        .route("/{id}", patch(update_post_handler).delete(delete_post_handler))
    ...
    let user_routes = Router::new()
        ...
        .route("/me/avatar", post(upload_avatar_handler).delete(delete_avatar_handler))
        .route("/me/preferences", get(get_preferences_handler).put(update_preferences_handler))
