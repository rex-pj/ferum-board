...
impl std::fmt::Debug for Config {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Config")
            ...
            .field("database_read_url", &self.database_read_url.as_ref().map(|_| "[redacted]"))
            ...
            .field("s3_secret_key", &self.s3_secret_key.as_ref().map(|_| "[redacted]"))
            .field("cdn_base_url", &self.cdn_base_url)
            .field("meilisearch_url", &self.meilisearch_url)
            .field("meilisearch_key", &self.meilisearch_key.as_ref().map(|_| "[redacted]"))
            ...
            .field("setup_admin_password", &self.setup_admin_password.as_ref().map(|_| "[redacted]"))
            .finish()
    }
}
...
fn default_cors() -> String { "http://localhost:5173".to_string() }
fn default_max_upload_mb() -> u64 { 5 }
