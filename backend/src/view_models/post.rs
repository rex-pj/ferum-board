...
impl From<Post> for PostResponse {
    fn from(p: Post) -> Self {
        let author = p.author_username.as_ref().map(|username| PostAuthorResponse {
        ...
        let reactions = p
            .reactions
            .into_iter()
            .map(|(kind, count)| ReactionCountResponse { kind: kind.as_str().to_string(), count })
            .collect();

        let my_reactions = p.my_reactions.iter().map(|k| k.as_str().to_string()).collect();
