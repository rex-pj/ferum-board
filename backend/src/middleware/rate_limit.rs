...
impl RateLimitConfig {
    pub fn auth() -> Self {
        Self { limit: 10, window: Duration::from_secs(60) }
    }

    pub fn public_write() -> Self {
        Self { limit: 30, window: Duration::from_secs(60) }
    }
}
...
pub async fn rate_limit_middleware(
    ...
    match state.rate_limiter.check(&key, config.limit, config.window).await {
