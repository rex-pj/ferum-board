...
impl CategoryUseCase {
    ...
    pub async fn get_by_slug(&self, actor: Option<&AuthUser>, slug: &str) -> Result<Category, AppError> {
        let category = self
            .categories
            .find_by_slug(slug)
            .await?
    ...
    pub async fn get_forum_index(
        ...
        let (counts_vec, recent_threads) = tokio::try_join!(
            ...
            self.threads.list_recent_by_categories(&parent_ids, FORUM_INDEX_THREADS_PER_CATEGORY),
        )?;
        ...
        for thread in recent_threads {
            threads_by_cat.entry(thread.category_id).or_default().push(thread);
        }
