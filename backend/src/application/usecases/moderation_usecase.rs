...
use crate::domain::models::report::{Report, ReportStatus};
...
impl ModerationUseCase {
    ...
    pub async fn list_pending_reports(
        ...
        self.reports.list_all(Some(ReportStatus::Pending), target_type, page, per_page.min(50)).await
    }
    ...
    pub async fn warn_user(
        ...
        let target_user = self.users.find_by_id(user_id).await?.ok_or(AppError::NotFound)?;
        if target_user.role >= actor.role && actor.role < UserRole::Admin {
            return Err(AppError::forbidden("cannot_warn_higher_or_equal_role"));
        }

        self.users.update(user_id, UpdateUser { warn_count_delta: Some(1), ..Default::default() }).await?;

        // Notify the user
        self.notifications.create(
            user_id,
            crate::domain::models::notification::NotificationKind::Warn,
            serde_json::json!({ "reason": reason, "by": actor.username }),
        ).await?;

        self.event_bus.publish(ForumEvent::UserWarned {
            user_id,
            by_user_id: actor.id,
            reason,
        }).await;
    ...
    pub async fn temp_ban(
        ...
        let target = self.users.find_by_id(user_id).await?.ok_or(AppError::NotFound)?;
        ...
        self.event_bus.publish(ForumEvent::UserBanned {
            user_id,
            by_user_id: actor.id,
            reason,
            until: Some(until),
