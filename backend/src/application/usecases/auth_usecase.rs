...
impl AuthUseCase {
    ...
    pub async fn login(&self, cmd: LoginCmd) -> Result<LoginResult, AppError> {
        ...
        if user_opt.is_none() {
            let _ = self.hasher.verify(&cmd.password, "$2b$12$invalidhashpaddinginvalidhashpa").await;
    ...
    pub async fn refresh_access_token(&self, refresh_token: &str) -> Result<RefreshResult, AppError> {
