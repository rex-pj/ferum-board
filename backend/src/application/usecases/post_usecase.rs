...
use crate::constants::{MAX_POST_CONTENT_BYTES, MAX_POSTS_PER_PAGE};
use crate::domain::events::ForumEvent;
use crate::domain::models::post::Post;
use crate::domain::models::thread::ThreadStatus;
...
impl PostUseCase {
    pub fn new(
        ...
        Self { posts, threads, categories, users, reactions, event_bus }
    }
    ...
    pub async fn list_by_thread(
        ...
        let category = self.categories.find_by_id(thread.category_id).await?.ok_or(AppError::NotFound)?;
        ...
        let (users, reaction_counts, my_reactions) = tokio::try_join!(
            ...
            async {
                match actor {
                    Some(a) => self.reactions.user_reactions_for_posts(a.id, &post_ids).await,
                    None => Ok(HashMap::new()),
                }
            },
        )?;
    ...
    pub async fn create(
        ...
        let thread = self.threads.find_by_id(cmd.thread_id).await?.ok_or(AppError::NotFound)?;
        ...
        if let Some(parent_id) = cmd.parent_id {
            let parent = self.posts.find_by_id(parent_id).await?.ok_or(AppError::NotFound)?;
            if parent.thread_id != cmd.thread_id {
                return Err(AppError::unprocessable("parent_id does not belong to this thread"));
            }
        }
        ...
        self.event_bus.publish(ForumEvent::PostCreated {
            post_id: post.id,
            thread_id: cmd.thread_id,
    ...
    pub async fn edit(
        ...
        self.posts.update_content(id, content_md, content_html, actor.id).await
    }
...
async fn render_content(md: &str) -> Result<String, crate::application::shared::AppError> {
    let md = md.to_owned();
    tokio::task::spawn_blocking(move || {
        crate::validators::markdown::render_and_sanitize(&md)
            .ok_or_else(|| crate::application::shared::AppError::unprocessable(
                "Post content exceeds the maximum allowed size",
            ))
    })
    .await
    .map_err(|_| crate::application::shared::AppError::internal("render task panicked"))?
}
