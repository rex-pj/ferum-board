...
use crate::application::ports::{CacheService, ForumJob, JobQueue};
...
use crate::domain::models::thread::{Thread, ThreadStatus};
...
use crate::domain::repositories::thread_repository::{NewThread, ThreadRepository, UpdateThread};
...
impl ThreadUseCase {
    pub fn new(
        ...
        Self { threads, categories, posts, jobs, stored_files, event_bus, cache }
    }

    // ── Private helpers ──────────────────────────────────────────────────────

    async fn find_live_thread(&self, id: Uuid) -> Result<Thread, AppError> {
        let thread = self.threads.find_by_id(id).await?.ok_or(AppError::NotFound)?;
        if thread.deleted_at.is_some() { return Err(AppError::NotFound); }
        Ok(thread)
    }
            ...
            match serde_json::from_str(&cached) {
                ...
                Err(_) => {
                    ...
                    if let Ok(json) = serde_json::to_string(&cats) {
                        self.cache.set(CACHE_KEY, &json, Duration::from_secs(60)).await.ok();
                    }
                    cats
                }
            }
    ...
    pub async fn list_by_category(
        ...
        let (threads, total) = self.threads.list_by_category(category.id, page, per_page).await?;
        let threads = threads
            .into_iter()
            .map(|mut t| {
    ...
    pub async fn list_by_author(
        ...
        let (mut threads, total) =
            self.threads.list_by_author(author_id, &visible_ids, page, per_page).await?;
        Self::enrich_threads(&mut threads, &category_map);
        Ok((threads, total))
    }
    ...
    pub async fn update_title(
        ...
        self.threads
            .update(id, UpdateThread { title: Some(title), ..Default::default() })
            .await
    }
    ...
    pub async fn pin(&self, actor: &AuthUser, id: Uuid, assigned: &[Uuid], pin: bool) -> Result<Thread, AppError> {
        let thread = self.threads.find_by_id(id).await?.ok_or(AppError::NotFound)?;
        PermissionChecker::can_moderate(actor, thread.category_id, assigned)?;
        self.threads.update(id, UpdateThread { is_pinned: Some(pin), ..Default::default() }).await
    }

    pub async fn lock(&self, actor: &AuthUser, id: Uuid, assigned: &[Uuid], lock: bool) -> Result<Thread, AppError> {
        let thread = self.threads.find_by_id(id).await?.ok_or(AppError::NotFound)?;
        PermissionChecker::can_moderate(actor, thread.category_id, assigned)?;
        let status = if lock { ThreadStatus::Locked } else { ThreadStatus::Open };
        self.threads.update(id, UpdateThread { status: Some(status), ..Default::default() }).await
    }
    ...
    pub async fn move_to(
        ...
        let thread = self.threads.find_by_id(id).await?.ok_or(AppError::NotFound)?;
        PermissionChecker::can_moderate(actor, thread.category_id, assigned)?;
        self.categories.find_by_id(target_category_id).await?.ok_or(AppError::NotFound)?;
        self.threads.update(id, UpdateThread { category_id: Some(target_category_id), ..Default::default() }).await
    }
    ...
    pub async fn mark_solved(
        ...
        let best_post = self.posts.find_by_id(best_answer_id).await?.ok_or(AppError::NotFound)?;
        if best_post.thread_id != id {
            return Err(AppError::unprocessable("best_answer must belong to this thread"));
        }

        let result = self.threads.update(id, UpdateThread {
            is_solved: Some(true),
            best_answer_id: Some(Some(best_answer_id)),
            ..Default::default()
        }).await?;

        self.event_bus.publish(ForumEvent::BestAnswerMarked {
    ...
    pub async fn set_thumbnail(
        ...
        if !validate_image_content_type(&content_type) {
            return Err(AppError::unprocessable("thumbnail must be jpeg, png, webp, or gif"));
        }
        if data.len() > MAX_THUMBNAIL_BYTES {
            return Err(AppError::unprocessable("thumbnail exceeds 10 MB size limit"));
        }
