...
impl CategoryRepository for PgCategoryRepository {
    ...
    async fn update(&self, id: Uuid, patch: UpdateCategory) -> Result<Category, AppError> {
        ...
        if let Some(v) = patch.name { active.name = Set(v); }
        if let Some(v) = patch.slug { active.slug = Set(v); }
        if let Some(v) = patch.description { active.description = Set(v); }
        if let Some(v) = patch.parent_id { active.parent_id = Set(v); }
        if let Some(v) = patch.position { active.position = Set(v); }
        if let Some(v) = patch.view_policy { active.view_policy = Set(view_policy_to_entity(&v)); }
        if let Some(v) = patch.post_policy { active.post_policy = Set(post_policy_to_entity(&v)); }
        if let Some(v) = patch.color { active.color = Set(v); }
        if let Some(v) = patch.updated_by_id { active.updated_by_id = Set(Some(v)); }
    ...
    async fn count_threads_by_categories(&self, ids: &[Uuid]) -> Result<Vec<(Uuid, u64)>, AppError> {
        ...
        Ok(rows.into_iter().map(|r| (r.id, r.thread_count as u64)).collect())
    }
}
