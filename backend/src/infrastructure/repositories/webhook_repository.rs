...
use crate::constants::WEBHOOK_MAX_FAILURES;
...
impl WebhookRepository for PgWebhookRepository {
    ...
    async fn find_subscribed(&self, event_type: &str) -> Result<Vec<Webhook>, AppError> {
        Ok(webhooks::Entity::find()
            ...
            .filter(Expr::cust_with_values("$1 = ANY(events)", [event_type.to_string()]))
            .all(&self.db)
            .await?
            .into_iter()
            .map(entity_to_domain)
    ...
    async fn update(&self, id: Uuid, upd: UpdateWebhook) -> Result<Webhook, AppError> {
        ...
        if let Some(url) = upd.url { active.url = Set(url); }
        if let Some(events) = upd.events { active.events = Set(events); }
        if let Some(secret) = upd.secret { active.secret = Set(Some(secret)); }
        if let Some(is_active) = upd.is_active { active.is_active = Set(is_active); }
    ...
    async fn record_success(&self, id: Uuid) -> Result<(), AppError> {
        webhooks::Entity::update_many()
            .col_expr(webhooks::Column::LastTriggeredAt, Expr::value(Utc::now().fixed_offset()))
