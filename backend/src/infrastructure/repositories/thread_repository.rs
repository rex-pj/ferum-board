...
impl ThreadRepository for PgThreadRepository {
    ...
    async fn list_feed(
        ...
        let mut values: Vec<Value> = category_ids
            .iter()
            .map(|id| (*id).into())
            .collect();
    ...
    async fn update(&self, id: Uuid, patch: UpdateThread) -> Result<Thread, AppError> {
        ...
        if let Some(v) = patch.title { active.title = Set(v); }
        if let Some(v) = patch.status { active.status = Set(domain_status_to_entity(&v)); }
        if let Some(v) = patch.is_pinned { active.is_pinned = Set(v); }
        if let Some(v) = patch.is_solved { active.is_solved = Set(v); }
        if let Some(v) = patch.best_answer_id { active.best_answer_id = Set(v); }
        if let Some(v) = patch.category_id { active.category_id = Set(v); }
        if let Some(v) = patch.deleted_by_id { active.deleted_by_id = Set(Some(v)); }
        if let Some(v) = patch.deleted_at { active.deleted_at = Set(Some(v.fixed_offset())); }
