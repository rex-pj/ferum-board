...
impl StoredFileRepository for PgStoredFileRepository {
    ...
    async fn upsert(
        ...
        stored_files::Entity::insert(model)
            .on_conflict(OnConflict::column(stored_files::Column::Key).do_nothing().to_owned())
            .exec(&self.db)
            .await
            .map(|_| ())
            .or_else(|e| {
                // RecordNotInserted = key already exists, that's fine
                if matches!(e, DbErr::RecordNotInserted) { Ok(()) } else { Err(AppError::internal(e.to_string())) }
            })
    }
