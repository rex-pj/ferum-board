...
use crate::application::ports::{CacheService, JobQueue, NotificationBus, RateLimiter, SearchService, StorageService};
...
#[cfg(feature = "s3")]
use crate::infrastructure::storage::S3StorageService;
#[cfg(feature = "meilisearch")]
use crate::infrastructure::search::MeilisearchService;
...
pub async fn build_app_state(config: &Config) -> anyhow::Result<AppState> {
    ...
    let sse_bus: Arc<dyn NotificationBus> =
        Arc::new(SseNotificationBus::new(broadcaster.clone()));
    ...
    let storage: Arc<dyn StorageService> = match &config.s3_endpoint {
        Some(endpoint) => {
            ...
            Arc::new(S3StorageService::new(endpoint, access_key, secret_key, bucket, "us-east-1", cdn_base).await)
        }
        None => {
            tracing::info!("S3_ENDPOINT not set — using database storage (files stored in PostgreSQL)");
    ...
    let cat_mod_repo: Arc<dyn crate::domain::repositories::CategoryModeratorRepository> =
    ...
    let stored_file_repo: Arc<dyn crate::domain::repositories::StoredFileRepository> =
    ...
    let (cache, rate_limiter_raw, job_queue, notification_bus): (
        ...
        Some(url) => {
            ...
            let executor = Arc::new(JobExecutor::new(
                email.clone(), config.app_url.clone(),
                storage.clone(), stored_file_repo.clone(), webhook_repo.clone(),
            ));
            let cache = RedisCacheService::new(url).await
            ...
            let rate_limiter = RedisRateLimiter::new(url).await
    ...
    let token_service: Arc<dyn crate::application::ports::TokenService> =
    ...
    let site_config = Arc::new(PgSiteConfigRepository::new(pg_write.clone()));
...
pub async fn maybe_run_headless_setup(config: &Config, state: &AppState) -> anyhow::Result<()> {
    ...
    state.setup.run_setup(
        crate::application::usecases::setup_usecase::RunSetupCmd {
