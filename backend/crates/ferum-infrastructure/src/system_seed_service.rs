...
impl PgSystemSeedService {
    ...
    pub async fn seed_system(&self) -> Result<(), AppError> {
        ...
        // Email templates are deliberately NOT seeded. `email_templates` holds
        // only what an admin has edited, so "a row exists" means "customised" —
        // the distinction the editor's badge is built on. Pre-filling it would
        // make every template read as customised, and because seeding is
        // insert-only, an install would also stay frozen on the copy shipped by
        // whichever release first booted, never receiving later improvements.
        // Unedited copy resolves from `email_template::EMAIL_TEMPLATE_DEFAULTS`.
