...
use crate::ports::{
    AccessTokenClaims, CacheService, ForumJob, JobQueue, PasswordHasher, TokenService,
};
...
impl AuthUseCase {
    ...
    pub async fn register(&self, cmd: RegisterCmd) -> Result<User, AppError> {
        ...
        let token = self
            .tokens
            .mint_email_token(user.id, "email_verification")?;
        self.jobs
    ...
    pub async fn verify_email(&self, token: &str) -> Result<(), AppError> {
        ...
        self.users
            .set_trust_level(user_id, TrustLevel::Basic)
            .await?;
    ...
    pub async fn login(&self, cmd: LoginCmd) -> Result<LoginResult, AppError> {
        ...
        // Password verification — always run even if user has no password set so
        // response time is identical whether the email exists or not.
        let hash = user
            .password_hash
            .as_deref()
            .unwrap_or("$2b$12$invalidhashpaddinginvalidhashpa");
        ...
        if user.password_hash.is_none() || !valid {
            ...
            if count >= ACCOUNT_LOCKOUT_ATTEMPTS {
                let until =
                    Utc::now() + chrono::Duration::minutes(ACCOUNT_LOCKOUT_DURATION_MINUTES as i64);
        ...
        let claims = AccessTokenClaims {
            ...
            exp: (Utc::now() + chrono::Duration::seconds(crate::constants::JWT_EXPIRY_SECS as i64))
                .timestamp(),
        };
    ...
    pub async fn refresh_access_token(
        ...
        // Verify the JWT signature and expiry first (cheap, no I/O).
        let user_id = self.tokens.verify_refresh_token(refresh_token)?;

        // Confirm the token is still in cache (not revoked by logout).
    ...
    pub async fn logout(&self, user_id: Uuid, refresh_token: &str) -> Result<(), AppError> {
        self.cache
            .del(&refresh_token_key(user_id, refresh_token))
            .await?;
        Ok(())
    }
    ...
    pub async fn reset_password(&self, cmd: ResetPasswordCmd) -> Result<(), AppError> {
        ...
        // Single-use: atomically claim the token (SET NX). If the key already
        // exists a concurrent request beat us to it — reject immediately.
        ...
        let claimed = self
            ...
            .set_nx(
                &used_key,
                "1",
                Duration::from_secs(PASSWORD_RESET_TOKEN_TTL_SECS + 60),
            )
            .await?;
        if !claimed {
