...
/// `TestDb` clones a template that has already been seeded, so the defaults are
/// expected to be present without this test seeding anything itself.
#[tokio::test]
async fn every_compiled_in_default_is_seeded() {
    let db = TestDb::new("email_tpl_seeded").await;
    let repo = PgEmailTemplateRepository::new(db.conn.clone());

    let stored = repo.list().await.expect("list stored templates");

    for d in EMAIL_TEMPLATE_DEFAULTS {
        let found = stored
            .iter()
            .find(|t| t.key == d.key && t.locale == d.locale)
            .unwrap_or_else(|| panic!("{} / {} was never seeded", d.key, d.locale));
        assert_eq!(found.subject, d.subject, "{} / {} subject", d.key, d.locale);
    }
}
...
async fn resolve_takes_the_first_locale_in_chain_order() {
    ...
    // `vi-VN` has no row; `vi` does. `Locale::fallback_chain` yields
    // ["vi-VN", "vi", "en"].
    let hit = repo
    ...
    // With the base removed from the chain, the same lookup lands on the
...
/// The rule that makes running the seeder on every boot safe. An admin's edited
/// subject must still be there afterwards.
#[tokio::test]
async fn seeding_again_never_overwrites_edited_copy() {
    ...
    let after = repo
        .find("email-verify", "en")
        .await
        .expect("find runs")
        .expect("the row is still there");
    assert_eq!(
        after.subject, "Edited by the admin",
...
#[tokio::test]
async fn the_layout_is_stored_like_any_other_template() {
    let db = TestDb::new("email_tpl_layout").await;
    let repo = PgEmailTemplateRepository::new(db.conn.clone());

    let layout = repo
        .resolve(LAYOUT_KEY, &chain(&["en"]))
        .await
        .expect("resolve runs")
        .expect("the layout ships a default");
    assert!(
        layout.body_html.contains("{{ content }}"),
