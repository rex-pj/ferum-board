...
//!   - Every `ThreadSort` × `ThreadFeedFilter` combination in `list_by_category` and `list_feed`
...
/// Every ordering crossed with every narrowing — the full 3 × 3, not one axis
/// at a time.
///
/// The cross product is the point. Sort and filter are now assembled by two
/// separate functions (`order_by_clause` and `filter_fragment`) whose outputs
/// are concatenated into one statement, so a combination neither was written
/// with in mind is exactly where a malformed clause would appear. Before the
/// split these five were mutually exclusive and only five shapes existed.
fn all_sort_modes() -> Vec<(String, ThreadFilter)> {
    let sorts = [
        ("Activity", ThreadSort::Activity),
        ("Newest", ThreadSort::Newest),
        ("MostReplies", ThreadSort::MostReplies),
    ];
    let filters = [
        ("All", ThreadFeedFilter::All),
    ...
    sorts
        .iter()
        .flat_map(|(sort_name, sort)| {
            filters.iter().map(move |(filter_name, filter)| {
                (
                    format!("{sort_name}+{filter_name}"),
                    ThreadFilter { sort: *sort, filter: *filter },
                )
            })
        })
        .collect()
...
async fn list_by_tag_join_executes_on_empty_schema() {
    ...
    let filter = ThreadFilter { sort: ThreadSort::Activity, filter: ThreadFeedFilter::All };
    let (threads, total) = repo
...
#[tokio::test]
async fn list_by_category_unanswered_filter_narrows_correctly() {
    let db = TestDb::new("thread_unanswered_filter").await;
    let repo = PgThreadRepository::new(db.conn.clone());
    ...
    let filter = ThreadFilter {
        ...
        filter: ThreadFeedFilter::Unanswered,
    };
    let (threads, total) = repo
        .list_by_category(cat.id, &filter, 1, 20, None)
        .await
        .expect("Unanswered filter with status enum cast executes");
...
#[tokio::test]
async fn list_by_category_solved_filter_narrows_correctly() {
    let db = TestDb::new("thread_solved_filter").await;
    let repo = PgThreadRepository::new(db.conn.clone());

    let user = make_user(&db.conn, 1).await;
    let cat = make_category(&db.conn, "help").await;
    // Neither thread is solved, so the filter must exclude both — the assertion
    // that matters is that it removes rows the unfiltered call returns, not that
    // the SQL merely runs.
    make_thread(&db.conn, cat.id, user.id, 1).await;
    make_thread(&db.conn, cat.id, user.id, 2).await;

    let unfiltered = ThreadFilter {
        sort: ThreadSort::Activity,
        filter: ThreadFeedFilter::All,
    };
    let (_, total_all) = repo
        .list_by_category(cat.id, &unfiltered, 1, 20, None)
        .await
        .expect("unfiltered listing executes");
    assert_eq!(total_all, 2);

    let solved = ThreadFilter {
        sort: ThreadSort::Activity,
        filter: ThreadFeedFilter::Solved,
    };
    let (threads, total) = repo
        .list_by_category(cat.id, &solved, 1, 20, None)
        .await
        .expect("Solved filter executes");

    assert_eq!(total, 0, "no thread is solved");
    assert!(threads.is_empty());

    db.teardown().await;
}

/// The COUNT and the data query build their WHERE from two different functions
/// (`filter_condition` in sea_query, `filter_fragment` in raw SQL). If those
/// drift, a listing reports a total that its own rows contradict — the
/// paginator then advertises pages that render empty, with no error anywhere.
#[tokio::test]
async fn count_and_rows_agree_under_every_filter() {
    let db = TestDb::new("thread_count_rows_agree").await;
    let repo = PgThreadRepository::new(db.conn.clone());

    let user = make_user(&db.conn, 1).await;
    let cat = make_category(&db.conn, "agree").await;
    for i in 1u8..=3 {
        make_thread(&db.conn, cat.id, user.id, i).await;
    }
    ...
    for (name, filter) in all_sort_modes() {
        let (threads, total) = repo
            .list_by_category(cat.id, &filter, 1, 20, None)
            .await
            .unwrap_or_else(|e| panic!("list_by_category {name} failed: {e:?}"));
        assert_eq!(
            threads.len() as u64,
            total,
            "{name}: COUNT said {total} but {} rows came back",
            threads.len()
        );
    }
