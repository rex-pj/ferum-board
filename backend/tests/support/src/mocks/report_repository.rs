...
mockall::mock! {
    ...
    impl ReportRepository for ReportRepository {
        ...
        async fn count_by_status<'a>(&self, category_ids: Option<&'a [Uuid]>) -> Result<ReportStatusCounts, AppError>;
        async fn list_all<'a>(&self, status: Option<ReportStatus>, target_type: Option<&'a str>, q: Option<&'a str>, category_ids: Option<&'a [Uuid]>, page: u64, per_page: u64) -> Result<(Vec<Report>, u64), AppError>;
        async fn list_by_reporter(&self, reporter_id: Uuid, page: u64, per_page: u64) -> Result<(Vec<Report>, u64), AppError>;
