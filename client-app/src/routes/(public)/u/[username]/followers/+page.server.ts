...
export const load: PageServerLoad = async ({ params, fetch, url, cookies }) => {
	...
	const [followersRes, statusRes] = await Promise.all([
		fetch(`${API}/api/users/${profile.id}/followers?page=${page}&per_page=20`),
	...
	return {
		...
		total: followersJson.meta?.total ?? 0,
		currentPage: followersJson.meta?.page ?? 1,
		perPage: followersJson.meta?.per_page ?? 20
	};
};
