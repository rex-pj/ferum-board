<svelte:head>
	<title>Following — {data.profile?.display_name ?? data.profile?.username ?? 'User'}</title>
</svelte:head>
	...
	<div class="fr-feed-col">
		...
		<div class="card mb-4 fr-profile-card">
			...
			<div class="px-3 px-sm-4 pb-3 pb-sm-4 fr-profile-body">
				...
				<div class="fr-profile-stats mt-3">
					...
					<!-- Active: not a link since we're on the Following page -->
					<div class="fr-profile-stat-item fr-stat-active">
						<span class="fw-semibold small" style="color:var(--bs-primary)">{followingCount.toLocaleString()}</span>
						<span class="fr-stat-label fr-stat-label-active">Following</span>
					</div>
		...
		<div class="d-flex justify-content-between align-items-center mb-3">
			<h2 class="h6 fw-semibold mb-0" style="color:var(--bs-secondary-color)">
				<i class="fa-solid fa-user-group me-2"></i>Following
				{#if data.total > 0}
		...
		{#if data.following.length > 0}
			<div class="fr-feed mb-3">
				{#each data.following as item (item.id)}
			...
			{#if totalPages > 1}
				<nav aria-label="Following pages">
					<ul class="pagination pagination-sm">
						{#if currentPage > 1}
							<li class="page-item">
								<a class="page-link" href="{ROUTES.USER_FOLLOWING(profile.username)}?page={currentPage - 1}">Previous</a>
							</li>
						{/if}
						{#each Array.from({ length: Math.min(totalPages, 10) }, (_, i) => i + 1) as p}
							<li class="page-item" class:active={p === currentPage}>
								<a class="page-link" href="{ROUTES.USER_FOLLOWING(profile.username)}?page={p}">{p}</a>
							</li>
						{/each}
						{#if currentPage < totalPages}
							<li class="page-item">
								<a class="page-link" href="{ROUTES.USER_FOLLOWING(profile.username)}?page={currentPage + 1}">Next</a>
							</li>
						{/if}
					</ul>
				</nav>
		...
		{:else}
			<div class="fr-feed">
				<div class="fr-empty-state">
					<div class="fr-empty-icon"><i class="fa-solid fa-user-group"></i></div>
					<h2 class="fr-empty-title">Not following anyone</h2>
					<p class="fr-empty-sub">This user isn't following anyone yet.</p>
				</div>
			</div>
		{/if}
	</div>
