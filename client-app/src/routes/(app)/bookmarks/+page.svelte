...
<div class="fr-content-layout">
	...
	<div class="fr-feed-col">
		<div class="d-flex justify-content-between align-items-center mb-3">
			<h1 class="h5 fw-semibold mb-0">
				<i class="fa-solid fa-bookmark me-2" style="color:var(--bs-primary);"></i>Bookmarks
			</h1>
			{#if total > 0}
				<span class="badge rounded-pill" style="background:var(--bs-tertiary-bg); color:var(--bs-secondary-color); border:1px solid var(--bs-border-color); font-weight:500;">
					{total}
				</span>
			{/if}
		</div>
		...
		{#if data.bookmarks?.length > 0}
			<div class="fr-feed mb-3">
				{#each data.bookmarks as item}
					<a href={ROUTES.THREAD(item.thread.slug)} class="fr-thread-row" style="text-decoration:none; display:flex;">
						<div style="flex:1; min-width:0;">
							<div class="fw-medium text-truncate" style="font-size:0.9375rem; color:var(--bs-body-color);">
								{#if item.thread.is_solved}
									<i class="fa-solid fa-circle-check text-success me-1" style="font-size:0.8rem;" title="Solved"></i>
								{/if}
								{#if item.thread.status === 'locked'}
									<i class="fa-solid fa-lock me-1" style="font-size:0.8rem; color:var(--bs-warning);" title="Locked"></i>
								{/if}
								{item.thread.title}
							</div>
							<div class="d-flex align-items-center gap-3 mt-1" style="font-size:0.75rem; color:var(--bs-tertiary-color);">
						...
						<i class="fa-solid fa-bookmark flex-shrink-0 mt-1" style="font-size:0.75rem; color:var(--bs-primary); opacity:0.6;"></i>
					</a>
				{/each}
			</div>
	...
	<aside class="fr-right-panel">
		...
		<div class="fr-panel">
			...
			<div class="fr-panel-body px-3 py-2">
				<div class="fr-panel-stat" style="border:none; padding:0.5rem 0;">
					<span style="color:var(--bs-secondary-color); font-size:0.8125rem;">Saved threads</span>
				...
				{#if totalPages > 1}
					<div class="fr-panel-stat" style="border-top:1px solid var(--bs-border-color); padding:0.5rem 0 0;">
						<span style="color:var(--bs-secondary-color); font-size:0.8125rem;">Page</span>
						<span class="fw-semibold" style="font-size:0.8125rem; color:var(--bs-body-color);">{currentPage} / {totalPages}</span>
					</div>
				{/if}
			</div>
		</div>
		...
		<div class="fr-panel">
			...
			<div class="fr-panel-body">
				<a href={ROUTES.NOTIFICATIONS} class="fr-panel-row">
					<i class="fa-solid fa-bell" style="width:1rem; text-align:center; font-size:0.75rem;"></i>
					Notifications
				</a>
		...
		<div class="fr-panel">
			...
			<div class="px-3 py-3">
				<p class="mb-0" style="font-size:0.8125rem; color:var(--bs-secondary-color); line-height:1.6;">
					Open any thread and click the <i class="fa-solid fa-bookmark mx-1" style="color:var(--bs-primary);"></i> icon to save or unsave it.
				</p>
			</div>
		</div>
	</aside>
