...
<script lang="ts">
	...
	let adminErrors = $derived({
		username:
			admin.username.length > 0 && (admin.username.length < 3 || admin.username.length > 30)
				? 'Username must be 3–30 characters'
				: '',
		passwordMismatch:
			admin.confirm.length > 0 && admin.password !== admin.confirm ? 'Passwords do not match' : '',
		passwordTooShort:
	...
	function buildPayload() {
		...
		if (hasConfig) {
			payload.config = {
				...
				...(config.site_tagline.trim() && { site_tagline: config.site_tagline }),
...
{#if step === 1}
	...
	<div class="alert alert-info d-flex align-items-start gap-2 mb-4">
		...
		<div>
			This wizard will create your admin account and configure the forum. It runs only once —
			after setup is complete, these pages are permanently disabled.
		</div>
	</div>
...
{:else if step === 2}
	...
	<p class="text-muted small mb-4">This will be the primary administrator of your forum.</p>
	...
	<div class="d-flex gap-2">
		...
		<button class="btn btn-primary flex-grow-1" disabled={!step2Valid} onclick={() => (step = 3)}>
...
{:else if step === 3}
	...
	<div class="row mb-3">
		<div class="col-md-6">
			...
			<div class="input-group">
				...
				/>
				<input type="text" class="form-control" bind:value={config.primary_color} />
			</div>
		</div>
		<div
		...
		>
			...
			>
				...
				/>
				<label class="form-check-label" for="registration_open">Open registration</label>
			</div>
		</div>
	</div>
	...
	>
		<i class="fa-solid {showSmtp ? 'fa-chevron-down' : 'fa-chevron-right'} me-1"></i>
		Email / SMTP settings
	</button>
	...
	<div class="d-flex gap-2">
		...
		<button class="btn btn-outline-primary" onclick={() => (step = 4)}>Skip</button>
...
{:else if step === 4}
	...
	<p class="text-muted small mb-4">
		Optionally install sample categories, threads, posts, and reactions to explore the forum.
	</p>
	...
	<div
		...
		style="cursor: pointer; background-color: {seedExampleData
			...
			: ''}; border-color: {seedExampleData ? 'var(--bs-primary)' : ''} !important;"
		role="button"
		tabindex="0"
		...
		<div class="d-flex align-items-start gap-3">
			...
			<div>
				...
				<div class="text-muted small">
					Creates 3 categories (Announcements, General, Feedback) with a sample thread, post, and
					reaction in each — great for exploring the interface before adding real content.
				</div>
			</div>
		</div>
	</div>
