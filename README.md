...
| `SECRET_ENCRYPTION_KEY` (+ `SECRET_ENCRYPTION_KEY_PREVIOUS`) | — | plaintext in the database | Encrypts the SMTP password and every webhook secret at rest (XChaCha20-Poly1305). 64 hex characters. Existing plaintext rows are converted on the next start; losing the key makes those two values unrecoverable, and nothing else in the database is encrypted. Rotation runbook in `docs/deployment.md`. |
