...
| `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS` | — | mail disabled | Outgoing email. These four are seeded into `site_config` on first startup and are editable afterwards at `/admin/settings`, applied live with no restart. TLS is inferred from the address: loopback in the clear, port 465 implicit TLS, anything else STARTTLS required. |
| `RESEND_API_KEY` | — | SMTP, or mail disabled | Send through Resend's HTTPS API instead. **Wins over the SMTP block** when both are set. Env-only on purpose — a key that can send mail as your domain does not belong in a table the settings page renders — so rotating it needs a restart. |
| `SECRET_ENCRYPTION_KEY` (+ `SECRET_ENCRYPTION_KEY_PREVIOUS`) | — | plaintext in the database, and **startup refuses when `APP_URL` is https** | Encrypts the SMTP password, every webhook secret and every plugin config field marked `secret = true` at rest (XChaCha20-Poly1305). 64 hex characters, **required on https**. Existing plaintext rows are converted on the next start; losing the key makes those values unrecoverable, and nothing else in the database is encrypted. Rotation runbook in `docs/deployment.md`. |
