...
export default defineHook(
  "before_post_create",
  async (ctx: BeforePostCreateContext) => {
    ...
    if (score >= Ferum.config.block_threshold) {
      ...
      return ctx.deny(
        "spam_detected",
        "Your post was flagged as spam. Please revise it.",
      );
    }
...
app.post("/events", express.json(), async (req, res) => {
  ...
  if (event_type === "thread.deleted") {
    ...
    await db.query("DELETE FROM rooms WHERE thread_id = $1", [
      payload.thread_id,
    ]);
  }
...
app.get("/api/plugins/chatbox/rooms/:threadId", async (req, res) => {
  ...
  const { rows } = await db.query("SELECT * FROM rooms WHERE thread_id = $1", [
    threadId,
  ]);
...
app.get("/api/plugins/chatbox/rooms/:roomId/messages", async (req, res) => {
  ...
  let query =
    "SELECT * FROM messages WHERE room_id = $1 AND is_deleted = false";
  const params = [roomId];
...
app.post(
  "/api/plugins/chatbox/rooms/:roomId/messages",
  express.json(),
  async (req, res) => {
    const userId = req.headers["x-ferum-user-id"];
...
const SOCKET_PATH =
  process.env.FERUM_PLUGIN_SOCKET || "/tmp/ferum-plugin-chatbox.sock";
...
class FerumChatboxThread extends HTMLElement {
  ...
  render() {
    ...
    if (this.currentUser) {
      this.querySelector("#chat-send").addEventListener("click", () =>
        this.sendMessage(),
      );
  ...
  async loadMessages() {
    const res = await fetch(
      `/api/plugins/chatbox/rooms/${this.roomId}/messages?limit=50`,
    );
  ...
  connectWebSocket() {
    ...
    const socket = io("/plugins/chatbox", {
      path: "/api/plugins/chatbox/socket.io",
    });
...
ferum-plugin pack
ferum-plugin install --forum-url=http://localhost:5173 \
...
```bash
curl -X POST http://localhost:5173/api/admin/plugins/debug/hooks \
...
1. Check the plugin is **Active** and the slot is registered:
   ```bash
   curl http://localhost:5173/api/plugins/active-slots | jq '.["thread.below_posts"]'
   ```
...
const res = await fetch(
  "http://localhost:5173/api/plugins/_internal/threads/abc123",
  {
    headers: {
